...
const openingRushConfig = {
  ...
  sStreakRewardEvery: 3,
  sStreakRewardShards: 2,
  buildLeaderboardRewardShards: 1,
  milestones: [
...
function createArchiveFallback() {
  return {
    ...
    openingRushSStreak: 0,
    bestOpeningRushSStreak: 0,
    ...
    completedOpeningRushBuildLeaderboardRewards: [],
...
function normalizeOpeningRushStreakReward(value) {
  if (!value || typeof value !== "object") {
    return null;
  }
  const amount = Math.max(0, Math.trunc(Number(value.amount) || 0));
  const streak = Math.max(0, Math.trunc(Number(value.streak) || 0));
  if (amount <= 0 || streak <= 0) {
    return null;
  }
  return {
    amount,
    ...
    score: clamp(Math.trunc(Number(value.score) || 0), 0, 100),
    grade: "S",
    reason: typeof value.reason === "string" ? value.reason : "开场S连胜",
    at: Number(value.at) || Date.now(),
  };
}

function normalizeOpeningRushBuildRewardIds(value = []) {
  if (!Array.isArray(value)) {
    return [];
  }
  return [...new Set(value.filter((id) => getStarterBuildById(id)))];
}

function normalizeOpeningRushBuildLeaderboardReward(value) {
  if (!value || typeof value !== "object") {
    return null;
  }
  const starterBuild = getStarterBuildById(value.starterBuildId);
  const amount = Math.max(0, Math.trunc(Number(value.amount) || 0));
  if (!starterBuild || amount <= 0) {
    return null;
  }
  return {
    amount,
    starterBuildId: starterBuild.id,
    starterBuildTitle: starterBuild.title,
    score: clamp(Math.trunc(Number(value.score) || 0), 0, 100),
    grade: "S",
    rank: Math.max(1, Math.trunc(Number(value.rank) || 1)),
    reason: typeof value.reason === "string" ? value.reason : "流派首榜",
    at: Number(value.at) || Date.now(),
  };
}
...
function normalizeLastRunReview(value) {
  ...
  return {
    ...
    openingRushBuildLeaderboardReward: normalizeOpeningRushBuildLeaderboardReward(value.openingRushBuildLeaderboardReward),
    at: Number(value.at) || Date.now(),
  };
}
...
function loadArchive() {
  ...
  try {
    ...
    const normalized = {
      ...
      completedOpeningRushBuildLeaderboardRewards: normalizeOpeningRushBuildRewardIds(saved.completedOpeningRushBuildLeaderboardRewards),
      lastOpeningRushBuildLeaderboardReward: normalizeOpeningRushBuildLeaderboardReward(saved.lastOpeningRushBuildLeaderboardReward),
    };
    ...
    normalized.openingRushSStreak = Math.max(0, Math.trunc(Number(normalized.openingRushSStreak) || 0));
    normalized.bestOpeningRushSStreak = Math.max(
      normalized.openingRushSStreak,
      Math.trunc(Number(normalized.bestOpeningRushSStreak) || 0),
    );
...
function saveArchive() {
  ...
  try {
    return writePlatformJson(ARCHIVE_STORAGE_KEY, archiveState);
  } catch {
...
function recordOpeningRushInArchive(rush = runStats?.openingRush) {
  ...
  archiveState.lastOpeningRushStreakReward = null;
  archiveState.lastOpeningRushBuildLeaderboardReward = null;
  ...
  if (snapshot.score >= 90) {
    archiveState.openingRushSStreak = Math.max(
      0,
      Math.trunc(Number(archiveState.openingRushSStreak) || 0),
    ) + 1;
    archiveState.bestOpeningRushSStreak = Math.max(
      Math.trunc(Number(archiveState.bestOpeningRushSStreak) || 0),
      archiveState.openingRushSStreak,
    );
  } else {
    archiveState.openingRushSStreak = 0;
  }
  if (snapshot.score >= 90 && chiefChallengeActive) {
    ...
    if (archiveState.openingRushSStreak > 0
      && archiveState.openingRushSStreak % openingRushConfig.sStreakRewardEvery === 0) {
      const streakAmount = grantCalibrationShards(openingRushConfig.sStreakRewardShards, "开场S连胜");
      streakReward = normalizeOpeningRushStreakReward({
        amount: streakAmount,
        streak: archiveState.openingRushSStreak,
        score: snapshot.score,
        grade: snapshot.grade,
        reason: "开场S连胜",
        at: Date.now(),
  ...
  let buildLeaderboardReward = null;
  archiveState.completedOpeningRushBuildLeaderboardRewards = normalizeOpeningRushBuildRewardIds(
    archiveState.completedOpeningRushBuildLeaderboardRewards,
  );
  if (
    buildLeaderboardEntry?.starterBuildId
    && buildLeaderboardEntry.rank === 1
    && buildLeaderboardEntry.score >= 90
    && !archiveState.completedOpeningRushBuildLeaderboardRewards.includes(buildLeaderboardEntry.starterBuildId)
  ) {
    const amount = grantCalibrationShards(
      openingRushConfig.buildLeaderboardRewardShards,
      `${buildLeaderboardEntry.starterBuildTitle}流派首榜`,
    );
    buildLeaderboardReward = normalizeOpeningRushBuildLeaderboardReward({
      amount,
      starterBuildId: buildLeaderboardEntry.starterBuildId,
      score: buildLeaderboardEntry.score,
      grade: buildLeaderboardEntry.grade,
      rank: buildLeaderboardEntry.rank,
      reason: "流派首榜",
      at: Date.now(),
    });
    if (buildLeaderboardReward) {
      archiveState.completedOpeningRushBuildLeaderboardRewards = [
        ...archiveState.completedOpeningRushBuildLeaderboardRewards,
        buildLeaderboardReward.starterBuildId,
      ];
      archiveState.lastOpeningRushBuildLeaderboardReward = buildLeaderboardReward;
      const completedBuildRewardCount = normalizeOpeningRushBuildRewardIds(
        archiveState.completedOpeningRushBuildLeaderboardRewards,
      ).length;
      if (completedBuildRewardCount >= starterBuilds.length) {
        unlockLocalAchievement("ACH_ALL_BUILD_FIRSTS", { playCue: false });
      }
    }
  }
  return {
...
function formatOpeningRushSStreak(archive = archiveState) {
  const current = Math.max(0, Math.trunc(Number(archive?.openingRushSStreak) || 0));
  const best = Math.max(current, Math.trunc(Number(archive?.bestOpeningRushSStreak) || 0));
  return current > 0 ? `x${current} · 最佳x${best}` : (best > 0 ? `暂无连胜 · 最佳x${best}` : "暂无连胜");
}

function getOpeningRushStreakPrompt(archive = archiveState) {
  if (!hasChiefOpeningChallenge(archive)) {
    return "";
  }
  const current = Math.max(0, Math.trunc(Number(archive?.openingRushSStreak) || 0));
  const every = Math.max(1, Math.trunc(Number(openingRushConfig.sStreakRewardEvery) || 3));
  const nextRewardStreak = current + (every - (current % every || every)) + (current % every === 0 ? every : 0);
  return `S连胜挑战：当前 x${current}，冲到 x${nextRewardStreak} 奖励 +${openingRushConfig.sStreakRewardShards} 碎片。`;
}
...
function formatOpeningRushBuildRewardProgress(archive = archiveState) {
  const claimed = normalizeOpeningRushBuildRewardIds(archive?.completedOpeningRushBuildLeaderboardRewards);
  return `${claimed.length}/${starterBuilds.length} 已领`;
}

function getNextOpeningRushBuildRewardHint(archive = archiveState) {
  const claimed = normalizeOpeningRushBuildRewardIds(archive?.completedOpeningRushBuildLeaderboardRewards);
  const nextBuild = starterBuilds.find((build) => !claimed.includes(build.id));
  if (!nextBuild) {
    return " · 流派首榜已全领";
  }
  return ` · 流派首榜 ${claimed.length}/${starterBuilds.length}：${nextBuild.title}+${openingRushConfig.buildLeaderboardRewardShards}碎片`;
}
...
function formatStarterBuildRewardLine(buildId, archive = archiveState) {
  const build = getStarterBuildById(buildId);
  if (!build) {
    return "";
  }
  const claimed = normalizeOpeningRushBuildRewardIds(archive?.completedOpeningRushBuildLeaderboardRewards);
  return claimed.includes(build.id)
    ? "首榜奖励：已领取"
    : `首榜奖励：首次S登顶 +${openingRushConfig.buildLeaderboardRewardShards}碎片`;
}
...
function createRunReview(victory) {
  ...
  return {
    ...
    openingRushBuildLeaderboardReward: normalizeOpeningRushBuildLeaderboardReward(archiveState?.lastOpeningRushBuildLeaderboardReward),
    at: Date.now(),
  };
}
...
function renderMetaProgression() {
  ...
  const chiefChallengeHint = hasChiefOpeningChallenge(archiveState)
    ...
    : "";
  const streakHint = hasChiefOpeningChallenge(archiveState)
    ? ` · S连胜 ${formatOpeningRushSStreak(archiveState)}`
    : "";
  const buildRewardHint = getNextOpeningRushBuildRewardHint(archiveState);
  ui.metaSummary.textContent = `碎片 ${shards} · 节点 ${totalLevels}/${maxLevels}${lastGain}${sBadgeSuggestion}${chiefChallengeHint}${streakHint}${buildRewardHint}`;
...
function renderStarterBuilds() {
  ...
  for (const build of starterBuilds) {
    ...
    const rewardLine = formatStarterBuildRewardLine(build.id, archiveState);
    button.innerHTML = `
      ${icon}
      <span class="choice-copy">
        ...
        <span class="specialization-line">${rewardLine}</span>
      </span>
    `;
    button.addEventListener("click", () => {
      playAudioCue("ui-confirm");
...
function renderStartMenu(options = {}) {
  ...
  archiveState = isStoreShotMode || options.preserveArchive ? archiveState : loadArchive();
  ...
  const streakPrompt = getOpeningRushStreakPrompt(archiveState);
  const chiefChallengeSuffix = [chiefChallengePrompt, streakPrompt].filter(Boolean).map((text) => ` ${text}`).join("");
...
function renderResultStats(victory) {
  ...
  const buildLeaderboardEntry = normalizeOpeningRushLeaderboardEntry(archiveState?.lastOpeningRushBuildLeaderboardEntry);
  const buildLeaderboardReward = normalizeOpeningRushBuildLeaderboardReward(archiveState?.lastOpeningRushBuildLeaderboardReward);
  const stats = [
    ...
    ["流派首榜", buildLeaderboardReward ? `+${buildLeaderboardReward.amount}碎片` : formatOpeningRushBuildRewardProgress(archiveState)],
...
function renderResultInsights(review = archiveState?.lastRunReview) {
  ...
  if (normalized.openingRushStreakReward) {
    cards.push([
      "S连胜",
      `开场 ${normalized.openingRushStreakReward.streak} 连 S`,
      `连续 S 级开场已达成阶段奖励，获得 +${normalized.openingRushStreakReward.amount} 校准碎片。断连会重置，继续保持榜首节奏。`,
      "is-boost",
    ]);
  }
  ...
  if (normalized.openingRushBuildLeaderboardReward) {
    cards.push([
      "流派首榜",
      `${normalized.openingRushBuildLeaderboardReward.starterBuildTitle} 已登顶`,
      `首次用这套流派拿到 S 级榜首，获得 +${normalized.openingRushBuildLeaderboardReward.amount} 校准碎片。下一把换另一套流派也有首榜奖励。`,
      "is-boost",
    ]);
  }
...
function installAutomationTestHooks() {
  ...
  function runChiefOpeningChallengeProbe() {
    ...
    archiveState = {
      ...
      bestChiefOpeningChallengeScore: 0,
      lastChiefOpeningChallengeReward: null,
      openingRushSStreak: 2,
      bestOpeningRushSStreak: 2,
      lastOpeningRushStreakReward: null,
    };
    saveArchive();
    world.mode = "menu";
    ...
    const buildLeaderboardEntry = normalizeOpeningRushLeaderboardEntry(archiveState.lastOpeningRushBuildLeaderboardEntry);
    const buildLeaderboardReward = normalizeOpeningRushBuildLeaderboardReward(archiveState.lastOpeningRushBuildLeaderboardReward);
    const archiveAfterRecord = cloneForSave(archiveState, null);
    const archiveSavedAfterRecord = saveArchive();
    const archiveReadBackAfterRecord = loadArchive();
    const archiveRoundTripAfterRecord = {
      saved: Boolean(archiveSavedAfterRecord),
      calibrationShards: archiveReadBackAfterRecord?.calibrationShards ?? 0,
      completions: archiveReadBackAfterRecord?.chiefOpeningChallengeCompletions ?? 0,
      streak: archiveReadBackAfterRecord?.openingRushSStreak ?? 0,
      leaderboardCount: archiveReadBackAfterRecord?.openingRushLeaderboard?.length ?? 0,
      buildRewardCount: normalizeOpeningRushBuildRewardIds(
        archiveReadBackAfterRecord?.completedOpeningRushBuildLeaderboardRewards,
      ).length,
    };
    archiveState = archiveAfterRecord ?? archiveState;
    world.mode = "menu";
    renderStartMenu({ preserveArchive: true });
    const startStatsAfter = ui.startStats?.textContent ?? "";
    const metaSummaryAfter = ui.metaSummary?.textContent ?? "";
    const starterBuildsAfter = ui.starterBuilds?.textContent ?? "";
    const review = normalizeLastRunReview({
      outcome: "defeat",
      chapterTitle: currentChapter().title,
      highlightTitle: rushBeforeArchive.gradeTitle,
      ...
      openingRushBuildLeaderboardReward: buildLeaderboardReward,
      at: Date.now(),
    });
    renderResultInsights(review);
    const insightsText = ui.resultInsights?.textContent ?? "";
    const result = {
      ok: hasChiefOpeningChallenge(archiveState)
        ...
        && startStatsBefore.includes("流派首榜")
        && startStatsBefore.includes("0/3")
        && metaSummaryBefore.includes("首席再巡")
        && metaSummaryBefore.includes("S连胜")
        && metaSummaryBefore.includes("x2")
        && metaSummaryBefore.includes("流派首榜 0/3")
        && startSummaryBefore.includes("S连胜挑战")
        && startSummaryBefore.includes("x3")
        ...
        && archiveState.bestOpeningRushSStreak === 3
        && archiveState.calibrationShards >= openingRushConfig.chiefChallengeRewardShards
          + openingRushConfig.sStreakRewardShards
          + openingRushConfig.buildLeaderboardRewardShards
        && leaderboardEntry?.rank === 1
        && leaderboardEntry?.score === 100
        && archived.buildLeaderboardEntry?.starterBuildId === "queue-barrage"
        && archived.buildLeaderboardReward?.starterBuildId === "queue-barrage"
        && archived.buildLeaderboardReward?.amount === openingRushConfig.buildLeaderboardRewardShards
        && buildLeaderboardEntry?.rank === 1
        && buildLeaderboardEntry?.starterBuildId === "queue-barrage"
        && buildLeaderboardReward?.starterBuildId === "queue-barrage"
        && buildLeaderboardReward?.amount === openingRushConfig.buildLeaderboardRewardShards
        && archiveState.openingRushBuildLeaderboards?.["queue-barrage"]?.length === 1
        && archiveState.completedOpeningRushBuildLeaderboardRewards.includes("queue-barrage")
        ...
        && metaSummaryAfter.includes("流派首榜 1/3")
        ...
        && insightsText.includes("流派首榜")
        && insightsText.includes(`+${openingRushConfig.buildLeaderboardRewardShards} 校准碎片`),
      startSummaryBefore,
      startStatsBefore,
      metaSummaryBefore,
      coachAfterS,
      ...
      archive: {
        ...
        buildLeaderboardRewards: cloneForSave(archiveState.completedOpeningRushBuildLeaderboardRewards, []),
      },
    };
    ui.resultInsights.innerHTML = "";
    deleteRunSave();
  ...
  function runOpeningRushBuildMasteryAchievementProbe() {
    const previousArchive = cloneForSave(archiveState, null);
    const previousRunStats = cloneForSave(runStats, null);
    const previousAchievements = [...readUnlockedAchievements()];
    writePlatformJson(
      ACHIEVEMENT_STORAGE_KEY,
      previousAchievements.filter((id) => id !== "ACH_ALL_BUILD_FIRSTS"),
    );
    resetStoreRun(0, { stepIndex: 0, bugPoints: 0, hp: 80, xp: 0, weaponIndex: 1 });
    archiveState = {
      ...createArchiveFallback(),
      calibrationShards: 0,
      bestOpeningRushScore: 95,
    ...
    const startStatsBefore = ui.startStats?.textContent ?? "";
    const metaSummaryBefore = ui.metaSummary?.textContent ?? "";
    const starterBuildsBefore = ui.starterBuilds?.textContent ?? "";
    runStats = createRunStats();
    runStats.openingRush = createOpeningRushState();
    runStats.openingSprint = createOpeningSprintState();
    runStats.starterBuild = "queue-barrage";
    ...
    const rushBeforeArchive = getOpeningRushSnapshot(runStats.openingRush);
    const archived = recordOpeningRushInArchive(runStats.openingRush);
    const buildReward = normalizeOpeningRushBuildLeaderboardReward(archiveState.lastOpeningRushBuildLeaderboardReward);
    const masteryAchievementUnlocked = readUnlockedAchievements().includes("ACH_ALL_BUILD_FIRSTS");
    const archiveAfterRecord = cloneForSave(archiveState, null);
    const archiveSavedAfterRecord = saveArchive();
    const archiveReadBackAfterRecord = loadArchive();
    const archiveRoundTripAfterRecord = {
    ...
    const review = normalizeLastRunReview({
      ...
      highlightTitle: rushBeforeArchive.gradeTitle,
      highlightText: rushBeforeArchive.gradeText,
      pressureTitle: "继续压缩三套流派榜",
      pressureText: "三套推荐流派都已经证明能在 30 秒内打出 S。",
      nextTitle: "三流派首榜",
      nextText: "三套首榜已全领，下一步可以冲总榜用时和 S 连胜。",
      recommendedStarterBuildId: "queue-barrage",
      openingRush: rushBeforeArchive,
      openingRushLeaderboardEntry: normalizeOpeningRushLeaderboardEntry(archiveState.lastOpeningRushLeaderboardEntry),
      openingRushBuildLeaderboardEntry: normalizeOpeningRushLeaderboardEntry(archiveState.lastOpeningRushBuildLeaderboardEntry),
      openingRushBuildLeaderboardReward: buildReward,
      at: Date.now(),
    });
    renderResultInsights(review);
    const insightsText = ui.resultInsights?.textContent ?? "";
    const claimedRewards = normalizeOpeningRushBuildRewardIds(archiveState.completedOpeningRushBuildLeaderboardRewards);
    const result = {
      ok: startStatsBefore.includes("流派首榜")
        && startStatsBefore.includes("2/3")
        && metaSummaryBefore.includes("流派首榜 2/3")
        && starterBuildsBefore.includes("键盘弹幕流")
        && starterBuildsBefore.includes("首次S登顶")
        && archived.buildLeaderboardReward?.starterBuildId === "queue-barrage"
        && buildReward?.amount === openingRushConfig.buildLeaderboardRewardShards
        && claimedRewards.length === starterBuilds.length
        && claimedRewards.includes("precision-breakpoint")
        && claimedRewards.includes("queue-barrage")
        && claimedRewards.includes("close-control")
        && masteryAchievementUnlocked
  ...
  function runRoutePressureTest() {
    ...
    const openingRushBuildMastery = runOpeningRushBuildMasteryAchievementProbe();
    ...
    if (!openingRushBuildMastery.ok) {
      failures.push("opening rush build mastery achievement failed");
    }
  ...
  window.__variableCityTestHooks = {
    ...
    runOpeningRushBuildMasteryAchievementProbe,
