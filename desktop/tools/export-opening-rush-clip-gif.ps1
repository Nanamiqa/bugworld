...
if (-not $Clip) {
  throw "store-page.json is missing reviewArtifacts.openingRushTrailerBoard.animatedClip"
}

$OutputPath = Join-Path $Root $Clip.path
$OutputDir = Split-Path -Parent $OutputPath
New-Item -ItemType Directory -Path $OutputDir -Force | Out-Null

$Frames = New-Object "System.Collections.Generic.List[System.Drawing.Bitmap]"
try {
  foreach ($FramePath in $Clip.frames) {
    $AbsoluteFramePath = Join-Path $Root $FramePath
    if (-not (Test-Path -LiteralPath $AbsoluteFramePath)) {
      throw "Missing opening rush clip frame: $FramePath"
    }
    $Frames.Add((New-ScaledFrame -Path $AbsoluteFramePath -Width ([int]$Clip.width) -Height ([int]$Clip.height))) | Out-Null
  }

  if ($Frames.Count -lt 6) {
    throw "The opening rush clip needs at least six frames"
  }
  ...
  $FirstFrame = $Frames[0]
  Set-GifTiming -Bitmap $FirstFrame -FrameCount $Frames.Count -FrameDelayMs ([int]$Clip.frameDelayMs)
